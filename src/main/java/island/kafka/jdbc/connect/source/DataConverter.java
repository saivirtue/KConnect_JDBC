...
public class DataConverter {
  ...
  private static void convertFieldValue(ResultSet resultSet, int col, int colType,
    ...
    switch (colType) {
      ...
      // Date is day + moth + year
      case Types.DATE: {
        colValue = resultSet.getDate(col, DateTimeUtils.LOCALE_CALENDAR.get());
        break;
      }

      // Time is a time of day -- hour, minute, seconds, nanoseconds
      case Types.TIME: {
        colValue = resultSet.getTime(col, DateTimeUtils.LOCALE_CALENDAR.get());
        break;
      }

      // Timestamp is a date + time
      case Types.TIMESTAMP: {
        colValue = resultSet.getTimestamp(col, DateTimeUtils.LOCALE_CALENDAR.get());
        break;
      }
