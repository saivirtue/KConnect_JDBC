package island.kafka.jdbc.connect.source;
...
import static island.kafka.jdbc.connect.source.JdbcSourceConnectorConfig.NumericMapping;
import island.kafka.jdbc.connect.util.DateTimeUtils;
import island.kafka.jdbc.connect.util.JdbcUtils;
  ...
  protected ResultSet executeQuery() throws SQLException {
    if (incrementingColumn != null && timestampColumn != null) {
      ...
      final long currentDbTime = JdbcUtils.getCurrentTimeOnDB(
          stmt.getConnection(),
          DateTimeUtils.LOCALE_CALENDAR.get()
      ).getTime();
      Timestamp endTime = new Timestamp(currentDbTime - timestampDelay);
      stmt.setTimestamp(1, endTime, DateTimeUtils.LOCALE_CALENDAR.get());
      stmt.setTimestamp(2, tsOffset, DateTimeUtils.LOCALE_CALENDAR.get());
      stmt.setLong(3, incOffset);
      stmt.setTimestamp(4, tsOffset, DateTimeUtils.LOCALE_CALENDAR.get());
      log.debug(
      ...
      Timestamp endTime = new Timestamp(currentDbTime - timestampDelay);
      stmt.setTimestamp(1, tsOffset, DateTimeUtils.LOCALE_CALENDAR.get());
      stmt.setTimestamp(2, endTime, DateTimeUtils.LOCALE_CALENDAR.get());
